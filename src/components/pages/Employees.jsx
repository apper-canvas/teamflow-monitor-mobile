...
...filteredEmployees.map(emp => [
  ...
  return (
    ...
    <div
        ...
        <div>
            <h1 className="text-2xl font-heading font-bold text-surface-900">Employees
                          </h1>
            <p className="text-surface-600 mt-1">Manage your team members and their information
                          </p>
        </div>
        <div className="flex items-center space-x-3 mt-4 sm:mt-0">
            <Button
                ...
                disabled={filteredEmployees.length === 0}>Export
                          </Button>
            <Button icon="UserPlus" onClick={() => navigate("/add-employee")}>Add Employee
                          </Button>
        </div>
    </div>
    {/* Search and Filters */}
...
{loading ? <div className="h-4 bg-surface-200 rounded w-24 animate-pulse"></div> : <>Showing {filteredEmployees.length} of {employees.length} employees
                                  </>}
            </div>
        </div>
    ...
    <motion.div
        ...
        {view === "grid" ? <EmployeeGrid
            employees={filteredEmployees}
            onEdit={handleEdit}
            onDelete={handleDeleteClick}
            loading={loading} /> : <EmployeeTable
            employees={filteredEmployees}
            onEdit={handleEdit}
            onDelete={handleDeleteClick}
            loading={loading} />}
